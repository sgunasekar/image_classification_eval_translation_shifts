...
def rand_augment_ops(magnitude=10, hparams=None, transforms=None):
    hparams = hparams or _HPARAMS_DEFAULT
    transforms = transforms or _RAND_TRANSFORMS
    print("RandTransform candidates", transforms)
    return [AugmentOp(
...
def rand_augment_transform(config_str, hparams):
    ...
    if not(tr):
        transforms = [t for t in transforms if not(t.startswith('Translate'))]
    if not(sh):
        transforms = [t for t in transforms if (not(t.startswith('Shear')) and not(t.startswith('Rotate')))]
